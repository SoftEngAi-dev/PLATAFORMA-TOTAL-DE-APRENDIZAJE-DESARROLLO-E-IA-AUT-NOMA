...
export default function App() {
  ...
  return <div className="studio-shell">
      ...
      <div className="content-grid">
        <section className="left-column">
          ...
          <div className="prompt-card">
            <div className="prompt-head"><div><span className="label">AGENTE</span><strong>Constructor principal</strong></div><select value={provider} onChange={(event: any) => setProvider(event.target.value as Provider)}><option value="ollama">Ollama local</option><option value="openai">OpenAI</option><option value="anthropic">Anthropic</option></select></div>
            <textarea value={prompt} onChange={(event: any) => setPrompt(event.target.value)} onKeyDown={(event: any) => { if ((event.ctrlKey || event.metaKey) && event.key === 'Enter') void runAgent(); }} placeholder="Ej.: crea un dashboard de ventas con autenticación, base de datos y tests…" />
    ...
    {showModal && <div className="modal-backdrop" onClick={() => setShowModal(false)}><div className="project-modal" onClick={(event: any) => event.stopPropagation()}><div className="modal-head"><div><span className="label">WORKSPACE</span><h3>Seleccionar proyecto</h3></div><button className="icon-btn" onClick={() => setShowModal(false)}><X size={17} /></button></div>{projects.map(item => <button key={item.id} className={item.name === project ? 'modal-project selected' : 'modal-project'} onClick={() => { setProject(item.name); setShowModal(false); }}><FolderOpen size={17} /><span>{item.name}</span></button>)}<button className="new-project" onClick={createProject}><Plus size={17} /> Crear proyecto</button></div></div>}
  </div>;
}